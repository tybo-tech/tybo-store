...
@Component({
  ...
  template: `
    <div
      class="category-slider-container relative overflow-hidden w-full h-[480px]"
    >
      <!-- Slides -->
          ...
          >
            ...
            <div class="absolute inset-0 flex items-center justify-center">
              <div class="w-full max-w-4xl mx-auto px-6">
                <div
                  class="bg-white/95 backdrop-blur-sm rounded-2xl p-8 md:p-12 shadow-2xl border border-white/20 text-center"
                >
                  <!-- Content -->
                  <h2
                    class="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 text-gray-900 leading-tight"
                  >
                    {{ slide.title }}
                  </h2>
                  <p
                    class="text-lg md:text-xl text-gray-600 mb-8 max-w-2xl mx-auto leading-relaxed"
                  >
                    {{ slide.subtitle }}
                  </p>
                  <a
      ...
      @if (content().length > 1) {
      <div
        class="dots-container absolute bottom-8 left-1/2 transform -translate-x-1/2 z-30"
      >
        <div
          class="flex space-x-3 bg-black/20 backdrop-blur-sm rounded-full px-4 py-2"
        >
          @for (slide of content(); track slide.link; let i = $index) {
          <button
            (click)="goToSlide(i)"
